...
public class gameTui {
  ...
  Vector<Card> viewHandCards; // cards in hand that are currently on screen
  ...
  Vector<Card> viewTableau; // cards in viewable Tableau

  Vector<Card> HabIsl; // current habitat and island cards
  ...
  public gameTui(Game game) throws IOException {
    ...
    terminal = new DefaultTerminalFactory()
                   .setInitialTerminalSize(new TerminalSize(tColSize, tRowSize))
                   .createTerminal();
    // screen size is hardcoded. The more dynamic layout managers make it
    // difficult to ensure that each element is in the correct location. I don't
    // see this as a big issue, but it could potentially be changed later with
    // some effort if needed.
    screen = new TerminalScreen(terminal);
    screen.startScreen();
    gameWindows = new MultiWindowTextGUI(screen, new DefaultWindowManager(),
    ...
    handCards = game.getActivePlayer().getHand(); // get cards from active
                                                  // player
    viewHandCards = new Vector<Card>(); // cards that are currently in view
  ...
  private class gameWindow extends BasicWindow {
    public gameWindow() { super("Player Name"); }
  }
  ...
  private void infoPanel() {
    ...
    drawBorder(infoStartCol, infoStartRow, infoColSize, infoRowSize,
               "Game Info");
  ...
  private void drawGrid() {
    ...
    drawBorder(gridCol, gridRow, miniCard.getCardColSize() * 4 + 2,
               miniCard.getCardRowSize() * 4 + 2, "Tableau");
  ...
  private void getHandView() {
    ...
    // changes how many cards to get if there are less cards in hand than the
    // max viewable
    if (handCards.size() < viewHandIndex + viewHandMax) {
  ...
  private void drawCardSlots(int startCol, int startRow, int cols, int rows,
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        ...
        curCard.setCardPosition(startCol + (miniCard.getCardColSize()) * j,
                                startRow + (miniCard.getCardRowSize()) * i);
        // sets card position
  ...
  private void drawBorder(int startCol, int startRow, int borderColSize,
                          int borderRowSize, String title) {
    ...
    borderContainer.addComponent(
        borderPanel.withBorder(Borders.singleLine(title)));
