...
public class Deck {
  ...
  void fillEarthDeck() throws FileNotFoundException {
    ...
    try {
      ...
      String name = list[0], habitat = list[1], abilities = list[2];
      int cost = Integer.parseInt(list[3]), vp = Integer.parseInt(list[4]);
      deckList.add(new TerrainCard(name, habitat, abilities, cost, vp));
      deck_file.close();
    } catch (Exception e) {
      System.out.println(e.toString());
    }
    // deckList.add(new TerrainCard("Prairie", "Sunny|Rocky|Cold", "Green:+1
    // Cards 3 Victory Points per Herb in this Row", 4, 2));
  }
  ...
  void fillClimateDeck() {
    ...
    try {
      ...
      String name = list[0], habitat = list[1], abilities = list[3];
  ...
  void fillFuanaDeck() {
    ...
    try {
      ...
      String name = list[0], objective = list[1];
